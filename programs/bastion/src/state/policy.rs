...
impl PolicyData {
  pub fn validate<'info>(
    ...
    use crate::policies::program_allowlist::{check_program_allowlist, check_program_blocklist};
    match self {
      ...
      PolicyData::NftCollectionAllowlist { collections } => {
        crate::policies::nft_collection::check_nft_collection_allowlist(collections, ix_accounts)
      }
      PolicyData::NftCollectionBlocklist { collections } => {
        crate::policies::nft_collection::check_nft_collection_blocklist(collections, ix_accounts)
      }
      ...
      PolicyData::NftCreatorAllowlist { creators } => {
        crate::policies::nft_creator_allowlist::check_nft_creator_allowlist(creators, ix_accounts)
      }
      ...
      PolicyData::NoAccountClose => crate::policies::no_account_close::check_no_account_close(
        &wrapped_ix.program_id,
        &wrapped_ix.data,
      ),
      ...
      PolicyData::MaxPriorityFee { max_micro_lamports } => {
        crate::policies::max_priority_fee::check_max_priority_fee(*max_micro_lamports, sysvar_ai)
      }
  ...
  pub fn normalize(&mut self) {
    match self {
      PolicyData::ProgramAllowlist { programs } | PolicyData::ProgramBlocklist { programs } => {
      ...
      PolicyData::IxDiscriminatorAllowlist { discriminators, .. } => discriminators.sort_unstable(),
      _ => {}
    }
  }
