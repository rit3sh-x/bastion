...
impl CounterState {
    ...
    pub fn charge_rolling(&mut self, now: i64, max: u32, secs: u32, slots: u8) -> Result<()> {
        ...
        // Fresh window: first-ever call, clock regression, or a full `secs`
        // window elapsed. The newest event always lands in the last slot.
        let elapsed = now.saturating_sub(self.last_reset);
        if self.last_reset == 0 || elapsed < 0 || elapsed >= secs_i {
            self.last_reset = now;
            self.ring = [0; MAX_RING_SLOTS];
            *self
                .ring
        ...
        // Advance the window by the WHOLE slots elapsed since the current slot
        // began, evicting ONLY buckets now strictly older than `secs` (a bucket
        // survives `slots` shifts == `secs`). Left-shift by `shifts`; the new
        // event lands in the current (last) slot.
        let shifts_raw = elapsed
            .checked_div(slot_duration)
            .ok_or(BastionError::InvalidWindow)?;
        let slots_i64 = i64::try_from(slots_u)?;
        let shifts = usize::try_from(core::cmp::min(shifts_raw, slots_i64))?;
        if shifts > 0 {
            for i in 0..slots_u {
                let src = i
                    .checked_add(shifts)
                    .ok_or(BastionError::NumericalOverflow)?;
                let v = if src < slots_u {
                    *self.ring.get(src).ok_or(BastionError::NumericalOverflow)?
                } else {
                    0
                };
                *self
                    .ring
                    .get_mut(i)
                    .ok_or(BastionError::NumericalOverflow)? = v;
            }
            let advance = i64::try_from(shifts)?
                .checked_mul(slot_duration)
                .ok_or(BastionError::NumericalOverflow)?;
...
impl SpendState {
    ...
    pub fn charge_rolling(
        ...
        // Fresh window: first-ever call, clock regression, or full `secs` elapsed.
        let elapsed = now.saturating_sub(self.last_reset);
        if self.last_reset == 0 || elapsed < 0 || elapsed >= secs_i {
            self.last_reset = now;
            self.ring = [0; MAX_RING_SLOTS];
        ...
        // Advance by whole slots elapsed; evict ONLY buckets strictly older than
        // `secs` (a bucket survives `slots` shifts == `secs`).
        let shifts_raw = elapsed
            .checked_div(slot_duration)
            .ok_or(BastionError::InvalidWindow)?;
        let slots_i64 = i64::try_from(slots_u)?;
...
mod tests {
    ...
    #[test]
    fn rolling_counter_no_premature_eviction_within_window() {
        // secs=60, slots=6 → slot_duration=10. A second event at
        // t+(secs-slot_duration)=t+50 is STILL inside the 60s window and must be
        // counted. With max=1 the second charge MUST be rejected (no ~2x bypass).
        let mut s = CounterState::default();
        s.charge_rolling(1_000, 1, 60, 6).unwrap();
        let res = s.charge_rolling(1_050, 1, 60, 6);
        assert_anchor_error(res, BastionError::RateLimitExceeded);
    }

    #[test]
    fn rolling_counter_window_slides_after_full_secs() {
        // A full `secs` after the first event the window resets and the slot is
        // free again — confirms eviction happens at the correct boundary.
        let mut s = CounterState::default();
        s.charge_rolling(1_000, 1, 60, 6).unwrap();
        s.charge_rolling(1_060, 1, 60, 6).unwrap();
        assert_eq!(s.count, 1);
    }

    #[test]
    fn rolling_spend_no_premature_eviction_within_window() {
        let mut s = SpendState::default();
        s.charge_rolling(1_000, 100, 100, 60, 6).unwrap();
        let res = s.charge_rolling(1_050, 100, 100, 60, 6);
