...
fn sol_spend_cap_rolling_window_slides_across_slots() {
    ...
    // 60s window / 2 slots (30s each), cap 1_000_000. One 400k spend in slot-0.
    svm.expire_blockhash();
    execute(
        &mut svm,
        &session_kp,
    ...
    .expect("slot-0 spend within cap");

    // Advance into the next slot; another 400k brings the window to 800k.
    ...
    .expect("slot-1 spend within cap");

    // A 3rd 400k is still inside the 60s window (1.2M > 1M cap) → rejected. The
    // pre-fix code slid at `secs - slot_duration`.
    svm.expire_blockhash();
    let res = execute(
        &mut svm,
        &session_kp,
    ...
    // A full window after the first slot: the slot-0 spend ages out (60s old),
    // reopening that budget — a new 400k settles (800k in window).
    advance_clock(&mut svm, 30);
    svm.expire_blockhash();
    execute(
        &mut svm,
    ...
    .expect("slot-0 spend aged out at t0+60 → budget reopens");
}
