...
fn extend_session_rejects_non_monotonic() {
  ...
  let err =
    extend_session(&mut svm, &owner, &session_pda, shrunk).expect_err("shrinking expiry must fail");
  let logs = err.meta.logs.join("\n");
  assert!(
...
fn extend_session_rejects_non_owner() {
  ...
  assert!(
    logs.contains("ConstraintSeeds") || logs.contains("0x7d6") || logs.contains("ConstraintHasOne"),
